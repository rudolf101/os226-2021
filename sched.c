...
static int bitmap_alloc(unsigned long *bitmap, size_t size) {
  int bit_pos;
  for (bit_pos = 0; (bitmap[bit_pos / sizeof(unsigned long)] & (1 << (bit_pos % sizeof(unsigned long)))) != 0
	  && bit_pos < MEM_PAGES; bit_pos++);
  if (bit_pos == MEM_PAGES) return -1;
  bitmap[bit_pos/sizeof(unsigned long)] |= 1 << (bit_pos % sizeof(unsigned long));
  return bit_pos;
}
